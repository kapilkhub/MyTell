...
namespace CreateUser
{
	public class CreateUserApiClient : ICreateUserApiClient
    {
		...
		public async Task<OneOf<CreateUserWithoutCredentialsOKResponse, CreateUserBadRequestResponse>> CreateUserWithoutCredentials(CreateUserWithoutCredentialsParameters queryParameters, CreateUserWithoutCredentialsRequest request, CancellationToken cancellationToken = default)
		{
			...
			{
				return await response.ReadJsonAsync<CreateUserBadRequestResponse>();
			}
		...
		public async Task<OneOf<CreateUserInGroupOKResponse,CreateUserBadRequestResponse>> CreateUserInGroup(CreateUserInGroupParameters queryParameters, CreateUserInGroupRequest request, CancellationToken cancellationToken = default)
		{
			var parametersDict = queryParameters.ToDictionary();
			var queryString = QueryHelpers.AddQueryString($"", parametersDict);
			var response = await _httpClient.PostAsJsonAsync(queryString, request, cancellationToken: cancellationToken);
			...
			if (response.StatusCode == HttpStatusCode.OK)
			{
				return await response.ReadJsonAsync<CreateUserInGroupOKResponse>();
			}
			throw new Exception($"CreateUserInGroup: Unexpected response. Status Code: {response.StatusCode}. Content: {await response.Content.ReadAsStringAsync()}");
		}
	}
}